...
def generateMIDIDICT():
    ...
    for i in range(0, 128) :    
        if octavenote == 12 : 
            octave+=1
            octavenote = 0
        notename = '{0}{1}'.format(Notes[octavenote] , str(octave))
        print '{0} = {1}'.format(notename,i)
        
        midiNotesDict[i] = notename
        
        octavenote += 1 
    return midiNotesDict


 
def getTunerValues(midifloat):
    
    #separate integer from fractional part 
      decimalpart, integerpart = math.modf(midifloat)
      if decimalpart > 0.5 :
          decimalpart -= 1
          integerpart += 1
          
      midinote = MIDIDICT[integerpart]  
      return midinote, decimalpart

def getChromaticNumber(midifloat, fullNotes = False):
    
    decimalpart, integerpart = math.modf(midifloat)
    if decimalpart > 0.5 :
        decimalpart -= 1
        integerpart += 1
    midiNote = int((integerpart % 12) + 1)
    
    if(fullNotes): 
        return midiNote 
    else :
        return float(midiNote) + decimalpart 